# H3VR_RUSSIAN_CORE
H3VR — базовый русский перевод (меню + спавнер) для версии "120 Patch 1 Build 24152369"
================================================
...
ПРО ФАЙЛЫ В ПАПКЕ Translation
------------------------------
_AutoGeneratedTranslations.txt — НЕ нужен в архиве.
  Плагин создаёт его сам при первом запуске. Туда попадают строки,
  которые ещё не переведены — для доработки русификатора.

_Substitutions.txt, _Preprocessors.txt, _Postprocessors.txt —
  служебные файлы плагина. Можно оставить пустыми; если их нет,
  плагин создаст сам.

Папка ru\ — создаётся автоматически при работе плагина.

ВАЖНО
-----
- Файл НЕ заменяет оригинальные .assets — перевод идёт через плагин
  во время игры.
- Если строка не переведена — останется английский. После игры
  смотрите _AutoGeneratedTranslations.txt и дополняйте H3VR_Russian_Core.txt.
